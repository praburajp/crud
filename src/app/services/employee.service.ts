...
export class EmployeeService {
  ...
  getCityList(): Observable<any> {
    const cityList = this.db.collection('departments').snapshotChanges().pipe(
      map(actions => {
        return actions.map(
          c => ({
  ...
  saveEmployee(employee: Employee) {
    ...
    return this.db.collection('Students').add(employeeData);
  }

  // Get the list of all employeee record from the Employee collection
  getAllEmployees(): Observable<any> {
    const employeeList = this.db.collection('Students').snapshotChanges().pipe(
      map(actions => {
        return actions.map(
          c => ({
  ...
  getEmployeeById(employeeId: string) {
    const employeeData = this.db.doc('Students/' + employeeId).valueChanges();
    return employeeData;
  }
  ...
  updateEmployee(employeeId: string, employee: Employee) {
    const employeeData = JSON.parse(JSON.stringify(employee));
    return this.db.doc('Students/' + employeeId).update(employeeData);
  }

  // Delete the employeee record from the Employee collection in firestore DB
  deleteEmployee(employeeId: string) {
    return this.db.doc('Students/' + employeeId).delete();
  }
}
